...
| `skills/lead-scanner/` | (1) Lens library: SQL-first anomaly scans that emit candidate leads. (2) Bill cross-check: a bill number **or** named alias → every Senate/House filing + press release touching it, each with a resolvable citation key (bridges press-names-bills vs filings-cite-numbers) | Lenses: `python queries/run_sweep.py db/lda.duckdb [BLOCK-PREFIX] [queries/<file>.sql]`. Bill cross-check: `python skills/lead-scanner/scripts/bill_lookup.py "Inflation Reduction Act"` (or `HR5376`; `--list-aliases`) |
