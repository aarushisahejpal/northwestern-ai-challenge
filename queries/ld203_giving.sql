-- LD-203 giving map — the citeable aggregate form of skills/lead-scanner/scripts/ld203_giving.py.
...
-- `ld203_giving.py <entity> --json` -> "ld203_filer_names").
--
-- Discipline baked in:
...
-- spellings included — see the ld203_giving.py --loose note on resolver name splits).
WITH dd AS (
  SELECT DISTINCT c.registrant_name, c.lobbyist_name, c.filer_type, c.filing_year,
         i.contribution_type, i.amount, i.payee, i.honoree, i.date, i.contributor_name
  FROM senate_contributions c JOIN senate_contribution_items i USING (filing_uuid)
